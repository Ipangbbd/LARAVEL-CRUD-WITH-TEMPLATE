...
@section('content')
    <h1 class="text-3xl font-bold text-blue-800 dark:text-blue-200 mb-6">Edit Post</h1>
    @if ($errors->any())
    ...
    @endif
    <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-900 p-4 sm:p-8 rounded shadow-md w-full max-w-xl mx-auto">
        @csrf
        @method('PUT')
        <div class="mb-5">
            <label for="title" class="block font-semibold text-gray-700 dark:text-gray-200 mb-2">Title</label>
            <input type="text" id="title" name="title" value="{{ old('title', $post->title) }}" required class="w-full border border-gray-300 dark:border-gray-700 rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:bg-gray-800 dark:text-gray-100 text-sm sm:text-base">
        </div>
        <div class="mb-8">
        ...
        <div class="mb-8">
            ...
            @if ($post->image_path)
                <div class="mt-2">
                    <img src="{{ asset('storage/' . $post->image_path) }}" alt="Current Image" class="h-20 w-20 object-cover rounded border">
                </div>
            @endif
            <p class="text-xs text-gray-500 mt-1">Upload to replace. Accepted: jpg, jpeg, png, webp. Max 10MB.</p>
        </div>
        <div class="mb-5">
            <label for="content" class="block font-semibold text-gray-700 dark:text-gray-200 mb-2">Content</label>
            <textarea id="content" name="content" required class="w-full border border-gray-300 dark:border-gray-700 rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300 min-h-[100px] dark:bg-gray-800 dark:text-gray-100 text-sm sm:text-base">{{ old('content', $post->content) }}</textarea>
        </div>
        <div class="mb-8">
            ...
            <select id="genre_id" name="genre_id" required class="w-full border border-gray-300 dark:border-gray-700 rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:bg-gray-800 dark:text-gray-100 text-sm sm:text-base">
                ...
                @foreach ($genres as $genre)
                    <option value="{{ $genre->id }}" {{ old('genre_id', $post->genre_id) == $genre->id ? 'selected' : '' }}>
                        {{ $genre->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2 rounded shadow transition w-full sm:w-auto">Update Post</button>
